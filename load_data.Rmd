...
exposures_yrprior <- co_yrprior %>%
  ...
  rename(metric = "lag") %>%
  mutate(date_exposure = as.POSIXct(strptime(paste(year, month, day, sep = "-"),
...
#lifetime exposures to air pollution, PAHs

```{r}
path <- "../C7_Lifetime_Exposures/PAH_EC_NO2_NOX_C7lifetime exposures_v1_noid.xlsx"

pah_lifetime <- read_xlsx(path = path,
                          sheet = 2) %>%
  rename_all(tolower) %>%
  rename("exposure_month" = month,
         "exposure_year" = year) %>%
  mutate(type = if_else(type == "0", "single", type)) %>%
  pivot_longer(cols = -c("id", "vis_date", "exposure_year", "exposure_month", "type"),
               names_to = "pollutant",
               values_to = "val") %>%
  mutate(metric = gsub("(meanmonth).*", "\\1", pollutant),
         pollutant = gsub("^meanmonth(.*)", "\\1", pollutant)) %>%
  rename(date_vis = "vis_date")
...
write_rds(pah_lifetime, "output_files/exposures_pah_lifetime.rds")
