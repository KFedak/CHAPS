...
* pre-bronchodilator test
```{r}
kable(head(pre_lf_long))
```

* post-bronchodilator test
```{r}
kable(head(post_lf_long))
```
...
# CO
co_yrprior <- read_xlsx(path = year_prior, sheet = 2) %>%
  rename_all(tolower) %>%
...
# ozone
o3_yrprior <- read_xlsx(path = year_prior, sheet = 3) %>%
  rename_all(tolower) %>%
  dplyr::rename(id = "subject") %>%
  pivot_longer(cols = -c("id", "month", "day", "year"),
...
#PM10
pm10_yrprior <- read_xlsx(path = year_prior, sheet = 4) %>%
  rename_all(tolower) %>%
  dplyr::rename(id = "subject") %>%
  pivot_longer(cols = -c("id", "month", "day", "year"),
...
#PM25
pm25_yrprior <- read_xlsx(path = year_prior, sheet = 5) %>%
  rename_all(tolower) %>%
  dplyr::rename(id = "subject") %>%
  pivot_longer(cols = -c("id", "month", "day", "year"),
...
```{r}
kable(head(exposures_30daylags))
```
...
```{r}
kable(head(exposures_avg_12months))
```
...
lifetime <- "../C7_Lifetime_Exposures/PAH_EC_NO2_NOX_C7lifetime exposures_v1_noid.xlsx"

exposures_lifetime <- read_xlsx(path = lifetime, sheet = 2) %>%
  rename_all(tolower) %>%
...
exposures_lifetime_co <- read_xlsx(path = lifetime2, sheet = 1) %>%
  rename_all(tolower) %>%
  rename("exposure_month" = month,
         "exposure_year" = year,
         "co" = 'co-monthly average 8-hr daily max concentration (ppm) -avg',
...
exposures_lifetime_pm25 <- read_xlsx(path = lifetime2, sheet = 2) %>%
  rename_all(tolower) %>%
  rename("exposure_month" = month,
         "exposure_year" = year,
         "pm25" = 'pm25-monthly average 24-hr concentration (ug/m3) -avg',
...
exposures_lifetime_pm10 <- read_xlsx(path = lifetime2, sheet = 3) %>%
  rename_all(tolower) %>%
  rename("exposure_month" = month,
         "exposure_year" = year,
         "pm10" = 'pm10-monthly average 24-hr concentration (ug/m3) -avg',
...
exposures_lifetime_o3 <- read_xlsx(path = lifetime2, sheet = 4) %>%
  rename_all(tolower) %>%
  rename("exposure_month" = month,
         "exposure_year" = year,
         "o3" = 'o3-monthly average of daily maximum 8-hr average  concentration (ppb) -avg',
...
```{r}
exposures_lifetime_ <- exposures_lifetime %>%
  left_join(exposures_lifetime_co, by = c("id", "date_visit", "exposure_year", "exposure_month", "type", "pollutant", "val")) %>%
  left_join(exposures_lifetime_o3, by = c("id", "date_visit", "exposure_year", "exposure_month", "type", "pollutant", "val")) %>%
  left_join(exposures_lifetime_pm25, by = c("id", "date_visit", "exposure_year", "exposure_month", "type", "pollutant", "val")) %>%
  left_join(exposures_lifetime_pm10, by = c("id", "date_visit", "exposure_year", "exposure_month", "type", "pollutant", "val"))
