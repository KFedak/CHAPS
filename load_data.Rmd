...
exposures_30daylags <- exposures_yrprior %>%
  ...
  mutate(lag_days = as.numeric(gsub("(lag)(.*)", "\\2", metric)),
         date_exp_measure = as.POSIXct((date_visit - (lag_days*24*60*60)))) #note that some have an issue with daylights savings time - will jump to hr 1:00:00 or 23:00:00...seems like this is OK though because it's not creating any issues where the lag days are duplicated - each 0-30 is assigned a unique date, 31 in succession. 
