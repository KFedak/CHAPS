...
```{r}
pah_lifetime <- read_xlsx("../C7_Lifetime_Exposures/PAH_EC_NO2_NOX_C7lifetime exposures_v1_noid.xlsx",
                              sheet = 2) %>%
              rename_all(tolower) %>%
              rename("exposure_month" = month,
                     "exposure_year" = year) %>%
              mutate(grep(ifelse(type == "0", "single", type))

unique(pah_lifetime$type)
%>%
              pivot_longer(cols = -c("id", "vis_date", "exposure_year", "exposure_month"),
                           names_to = "pollutant", values_to = "val")
