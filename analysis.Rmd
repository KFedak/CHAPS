...
basis.pol <- crossbasis(testdlnmdata$val, lag=30, 
#transformation on "val" which contains the numeric PM2.5 exposure value daily
                        argvar=list(fun="ns", df=6), 
#this is the argument that gets passed internally to function onebasis to generate the basis matrix for the predictor variable (val). chose natural cubic spline with 6 df (not sure why those choices...just copying examples)
...
model <- glm(value ~ basis.pol + age_months + height_cm^2 + weight_kg + sex + race_cat + asthma_ever, data = testdlnmdata)
# didn't specify the family object for the glm

# step 2 predict the dlnm

pred.pol <- crosspred(basis.pol, model, cen=25) # centering value automatically sets to 25 - not sure why or how to choose something else

# step 3 represent the dlnm (plot)
plot(pred.pol, type="p", pch=19, cex=1.5, ci="bars", col=2,
  ylab="RR",main="Lag-specific effects")

plot(pred.pol, "overall", ci="lines", ylim=c(0,5), lwd=2, col=4,
  xlab="PM2.5", ylab="RR", main="Overall effect")
...
plot(pred.pol, ci="n",ylim=c(0.95,1.22), lwd=1.5, col=2)
for(i in 1:2) lines(pred.pol, "slices", var=c(0,47)[i], col=i+2, lwd=1.5)
legend("topright",paste("PM2.5 =",c(-20,0,32)), col=2:4, lwd=1.5)
## don't understand this one but it's not working anyway (left as defaults of example)
# Error in lines.crosspred(pred.pol, "slices", var = c(0, 47)[i], col = i + : 'var' must match values used for prediction

plot(pred.pol,var=c(0,47), lag=c(0,47), ci.level=0.95, col=2, 
  xlab="PM2.5", ci.arg=list(density=20,col=grey(0.7)))
# don't understand this one but it's not working anyway (left as defaults of example)
#Error in plot.crosspred(pred.pol, var = c(0, 47), lag = c(0, 47), ci.level = 0.95,: 'var' must match values used for prediction
