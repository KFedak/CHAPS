...
```{r}
# step 0 choose 1 pollutant, 1 health effect only
testdlnmdata <- lf_pre_participants_expdaily %>%
  filter(pollutant == "pm25_ugm3" & var == "fvc")

# step 1 specify the DLNM
klag <- exp(((1+log(30))/4 * 1:3)-1) #not sure why example did these as the knots

basis.pol <- crossbasis(testdlnmdata$val, lag=30, #transformation on "val" which contains the numeric exposure value
                        argvar=list(fun="bs", degree=3, df=6), #not sure what these do...
                        arglag=list(knots=klag)) #not sure what this does


summary(basis.pol)

model <- glm(value ~ basis.pol + age_months + height_cm^2 + weight_kg + sex + race_cat + asthma_ever, # + ns(lag_days,7*14),
  family=quasipoisson(), testdlnmdata)

# step 2 predict the dlnm

pred.pol <- crosspred(basis.pol, model, cen=25) # centering value automatically sets to 25 - not sure why
...
plot(pred.pol, ci="n",ylim=c(0.95,1.22), lwd=1.5, col=2) # don't understand this one but it's not working anyway
for(i in 1:2) lines(pred.pol, "slices", var=c(0,47)[i], col=i+2, lwd=1.5)
legend("topright",paste("PM2.5 =",c(-20,0,32)), col=2:4, lwd=1.5)

plot(pred.pol,var=c(0,47), lag=c(0,47), ci.level=0.95, col=2, # don't understand this one but it's not working anyway
