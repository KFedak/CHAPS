...
pm_fvc_30day_dlnm <- lf_pre_participants_expdaily %>%
  filter(var == "fvc" & pollutant == "pm25_ugm3")
```
...
basis.pol <- crossbasis(pm_fvc_30day_dlnm$val, 
                        lag=30, 
...
```{r}
model_fvc <- glm(value ~ basis.pol + age_months + ns(height_cm) + weight_kg + sex + race_cat + asthma_ever, data = pah_fvc_30day_dlnm)
```
...
for(i in 1:4) lines(pred.pol, "slices", var=c(10,20,50,100)[i], col=i+2, lwd=1.5)
legend("topright", paste("PAH =",c(10,20,50,100)), col=2:4, lwd=1.5)
...
plot(pred.pol, var=c(10,20,50,100, 120), lag=c(0,1,7,14, 30), ci.level=0.95, col=2, 
...
```{r}
# step 0 - limit health effect and pollutant
pah_fev_30day_dlnm <- lf_pre_participants_expdaily %>%
  filter(var == "fev1" & pollutant == "pm25_ugm3")
```

### step 1 specify

```{r}
basis.pol <- crossbasis(pah_fev_30day_dlnm$val, 
                        lag=30, 
#transformation on "val_pol" which contains the numeric PAH exposure value daily
                        argvar=list(fun="ns", df=3), #, int = TRUE), 
#this is the argument that gets passed internally to function onebasis to generate the basis matrix for the predictor variable (val). chose natural cubic spline with 3 df (3 just because - do we have a justification for a different number?)
...
```{r}
model_fev <- glm(value ~ basis.pol + age_months + ns(height_cm) + weight_kg + sex + race_cat + asthma_ever, data = pah_fev_30day_dlnm)
```
...
```{r}
# step 0 - limit health effect and pollutant
pah_fef_30day_dlnm <- lf_pre_participants_expdaily %>%
  filter(var == "fef2575" & pollutant == "pm25_ugm3")
```

### step 1 specify

```{r}
basis.pol <- crossbasis(pah_fef_30day_dlnm$val, 
                        lag=30, 
#transformation on "val_pol" which contains the numeric PAH exposure value daily
                        argvar=list(fun="ns", df=3), #, int = TRUE), 
#this is the argument that gets passed internally to function onebasis to generate the basis matrix for the predictor variable (val). chose natural cubic spline with 3 df (3 just because - do we have a justification for a different number?)
...
```{r}
model_fef <- glm(value ~ basis.pol + age_months + ns(height_cm) + weight_kg + sex + race_cat + asthma_ever, data = pah_fef_30day_dlnm)
```
